...
class DoorRevisedConfirmationTable extends StatefulWidget {
  final String? dealerId;
  final String? dealerName;
  const DoorRevisedConfirmationTable(
      {super.key, this.dealerId, this.dealerName});

  @override
  State<DoorRevisedConfirmationTable> createState() =>
      _DoorRevisedConfirmationTableState();
}

class _DoorRevisedConfirmationTableState
    extends State<DoorRevisedConfirmationTable> {
    ...
    return FutureBuilder<List<OrdersModel>>(
      ...
      builder: (context, snapshot) {
        ...
        List<OrdersModel>? displayData =
            filteredList.isNotEmpty ? filteredList : doorRevisedList;
        ...
        return Consumer<PaginationProvider>(builder: (context, value, child) {
          return ClipRRect(
            ...
            child: PaginatedDataTable(
                rowsPerPage:
                    (doorRevisedList.length >= 5 && doorRevisedList.isNotEmpty)
                        ? 5
                        : (doorRevisedList.isEmpty)
                            ? 1
                            : doorRevisedList.length,
...
class MyData extends DataTableSource {
  List<OrdersModel> doorRevisedList;
  final String? dealerId;
  final String? dealerName;
  NetworkApiServices apiServices = NetworkApiServices();
  DateTime _datetime = DateTime.now();
  ...
  MyData(this.data, this.doorRevisedList, this._datetime, this.dealerId,
      this.dealerName, this._showDatePicker,
      {required this.myGlobalBuildContext});

  @override
  int get rowCount => doorRevisedList.length;
  ...
  DataRow getRow(int index) {
    ...
    final OrdersModel result = doorRevisedList[index];
