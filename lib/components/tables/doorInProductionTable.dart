...
class DoorInProductionTable extends StatefulWidget {
  final String? dealerId;
  final String? dealerName;
  const DoorInProductionTable({super.key, this.dealerId, this.dealerName});

  @override
  State<DoorInProductionTable> createState() => _DoorInProductionTableState();
}

class _DoorInProductionTableState extends State<DoorInProductionTable> {
      ...
      builder: (context, snapshot) {
        ...
        return Consumer<PaginationProvider>(builder: (context, value, child) {
          return PaginatedDataTable(
              ...
              source: MyData(list, _dateTime, inProductionList, widget.dealerId,
...
class MyData extends DataTableSource {
  List<OrdersModel> inProductionList;
  final String? dealerId;
  final String? dealerName;
  NetworkApiServices apiServices = NetworkApiServices();
  DateTime _datetime = DateTime.now();
  ...
  MyData(this.data, this._datetime, this.inProductionList, this.dealerId,
      this.dealerName, this._showDatePicker,
      {required this.myGlobalBuildContext});

  @override
  int get rowCount => inProductionList.length;
  ...
  DoorInProductionTable table = DoorInProductionTable();
  ...
  DataRow getRow(int index) {
    ...
    final OrdersModel result = inProductionList[index];

    //Widget selectedTable = determineTable(result, dealerId!);
    ValueKey key = ValueKey(result.id);
    return DataRow(
      key: key,
      cells: <DataCell>[
