...
class DoorAwaitingBalanceTable extends StatefulWidget {
  final String? dealerId;
  final String? dealerName;
  const DoorAwaitingBalanceTable({super.key, this.dealerId, this.dealerName});

  @override
  State<DoorAwaitingBalanceTable> createState() =>
      _DoorAwaitingBalanceTableState();
}

class _DoorAwaitingBalanceTableState extends State<DoorAwaitingBalanceTable> {
      ...
      builder: (context, snapshot) {
        ...
        return Consumer<PaginationProvider>(builder: (context, value, child) {
          return PaginatedDataTable(
              rowsPerPage: (awaitingBalanceList.length >= 5 &&
                      awaitingBalanceList.isNotEmpty)
                  ? 5
                  : (awaitingBalanceList.isEmpty)
                      ? 1
                      : awaitingBalanceList.length,
              ...
              source: MyData(list, awaitingBalanceList, _dateTime,
...
class MyData extends DataTableSource {
  List<OrdersModel> awaitingBalanceList;
  final String? dealerId;
  final String? dealerName;
  NetworkApiServices apiServices = NetworkApiServices();
  DateTime _datetime = DateTime.now();
  ...
  MyData(this.data, this.awaitingBalanceList, this._datetime, this.dealerId,
      this.dealerName, this._showDatePicker,
      {required this.myGlobalBuildContext});

  @override
  int get rowCount => awaitingBalanceList.length;
  ...
  DataRow getRow(int index) {
    ...
    final OrdersModel result = awaitingBalanceList[index];
