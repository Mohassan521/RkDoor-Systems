  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<OrdersModel>>(
      ...
      builder: (context, snapshot) {
        ...
        List<OrdersModel> awaitingDepositList = list
            .where((result) => result.orderStatusVal == "Awaiting Deposit")
            .toList();
        ...
        List<OrdersModel>? displayData =
            filteredList.isNotEmpty ? filteredList : awaitingDepositList;
        ...
        return Consumer<PaginationProvider>(builder: (context, value, child) {
          return ClipRRect(
            ...
            child: PaginatedDataTable(
                rowsPerPage: (awaitingDepositList.length >= 5 &&
                        awaitingDepositList.isNotEmpty)
                    ? 5
                    : (awaitingDepositList.isEmpty)
                        ? 1
                        : awaitingDepositList.length,
...
class MyData extends DataTableSource {
  List<OrdersModel> awaitingDeposit;
  final String? dealerId;
  final String? dealerName;
  NetworkApiServices apiServices = NetworkApiServices();
  DateTime _datetime = DateTime.now();
  ...
  MyData(this.data, this.awaitingDeposit, this._datetime, this.dealerId,
      this.dealerName, this._showDatePicker,
      {required this.myGlobalBuildContext});

  @override
  int get rowCount => awaitingDeposit.length;
  ...
  DataRow getRow(int index) {
    ...
    final OrdersModel result = awaitingDeposit[index];
