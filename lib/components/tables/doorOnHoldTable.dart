...
class DoorOnHoldTable extends StatefulWidget {
  final String? dealerId;
  final String? dealerName;
  const DoorOnHoldTable({super.key, this.dealerId, this.dealerName});

  @override
  State<DoorOnHoldTable> createState() => _DoorOnHoldTableState();
}

class _DoorOnHoldTableState extends State<DoorOnHoldTable> {
    ...
    return FutureBuilder<List<OrdersModel>>(
      ...
      builder: (context, snapshot) {
        ...
        List<OrdersModel>? displayData =
            filteredList.isNotEmpty ? filteredList : doorOnHoldList;
        ...
        return Consumer<PaginationProvider>(builder: (context, value, child) {
          return ClipRRect(
            ...
            child: PaginatedDataTable(
                rowsPerPage:
                    (doorOnHoldList.length >= 5 && doorOnHoldList.isNotEmpty)
                        ? 5
                        : (doorOnHoldList.isEmpty)
                            ? 1
                            : doorOnHoldList.length,
...
class MyData extends DataTableSource {
  List<OrdersModel> doorOnHoldList;
  final String? dealerId;
  final String? dealerName;
  NetworkApiServices apiServices = NetworkApiServices();
  DateTime _datetime = DateTime.now();
  ...
  MyData(this.data, this.doorOnHoldList, this._datetime, this.dealerId,
      this.dealerName, this._showDatePicker,
      {required this.myGlobalBuildContext});

  @override
  int get rowCount => doorOnHoldList.length;
  ...
  DataRow getRow(int index) {
    ...
    final OrdersModel result = doorOnHoldList[index];
