...
class DoorDeliveredTable extends StatefulWidget {
  final String? dealerId;
  final String? dealerName;
  const DoorDeliveredTable({super.key, this.dealerId, this.dealerName});

  @override
  State<DoorDeliveredTable> createState() => _DoorDeliveredTableState();
}

class _DoorDeliveredTableState extends State<DoorDeliveredTable> {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<List<OrdersModel>>(
      ...
      builder: (context, snapshot) {
        ...
        List<OrdersModel> doorDeliveredList = list
            .where((result) => result.orderStatusVal == "Delivered")
            .toList();

        return Consumer<PaginationProvider>(builder: (context, value, child) {
          return PaginatedDataTable(
              rowsPerPage: (doorDeliveredList.length >= 5 &&
                      doorDeliveredList.isNotEmpty)
                  ? 5
                  : (doorDeliveredList.isEmpty)
                      ? 1
                      : doorDeliveredList.length,
              ...
              source: MyData(list, doorDeliveredList, _dateTime,
...
class MyData extends DataTableSource {
  List<OrdersModel> doorDeliveredList;
  final String? dealerId;
  final String? dealerName;
  NetworkApiServices apiServices = NetworkApiServices();
  DateTime _datetime = DateTime.now();
  ...
  MyData(this.data, this.doorDeliveredList, this._datetime, this.dealerId,
      this.dealerName, this._showDatePicker,
      {required this.myGlobalBuildContext});

  @override
  int get rowCount => doorDeliveredList.length;
  ...
  DataRow getRow(int index) {
    ...
    final OrdersModel result = doorDeliveredList[index];
    ...
    //Widget selectedTable = determineTable(result, dealerId!);
      ...
      return DataRow.byIndex(
        ...
        cells: <DataCell>[
          ...
          DataCell(Text(
              result.facConfDocuments!.map((e) => e.toString()).join(', '))),
