...
class DoorTransitToUKTable extends StatefulWidget {
  final String? dealerId;
  final String? dealerName;
  const DoorTransitToUKTable({super.key, this.dealerId, this.dealerName});

  @override
  State<DoorTransitToUKTable> createState() => _DoorTransitToUKTableState();
}

class _DoorTransitToUKTableState extends State<DoorTransitToUKTable> {
      ...
      builder: (context, snapshot) {
        ...
        return Consumer<PaginationProvider>(builder: (context, value, child) {
          return PaginatedDataTable(
              rowsPerPage:
                  (transitToUkList.length >= 5 && transitToUkList.isNotEmpty)
                      ? 5
                      : (transitToUkList.isEmpty)
                          ? 1
                          : transitToUkList.length,
              ...
              source: MyData(list, transitToUkList, _dateTime, widget.dealerId,
...
class MyData extends DataTableSource {
  List<OrdersModel> transitToUkList;
  final String? dealerId;
  final String? dealerName;
  NetworkApiServices apiServices = NetworkApiServices();
  DateTime _datetime = DateTime.now();
  ...
  MyData(this.data, this.transitToUkList, this._datetime, this.dealerId,
      this.dealerName, this._showDatePicker,
      {required this.myGlobalBuildContext});

  @override
  int get rowCount => transitToUkList.length;
  ...
  DataRow getRow(int index) {
    ...
    final OrdersModel result = transitToUkList[index];
