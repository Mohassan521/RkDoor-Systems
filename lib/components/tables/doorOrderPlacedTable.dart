...
class DoorOrderPlacedTable extends StatefulWidget {
  final String? dealerId;
  final String? dealerName;
  const DoorOrderPlacedTable({super.key, this.dealerId, this.dealerName});

  @override
  State<DoorOrderPlacedTable> createState() => _DoorOrderPlacedTableState();
}

class _DoorOrderPlacedTableState extends State<DoorOrderPlacedTable> {
    ...
    return FutureBuilder<List<OrdersModel>>(
      ...
      builder: (context, snapshot) {
        ...
        List<OrdersModel>? displayData =
            filteredList.isNotEmpty ? filteredList : orderPlacedList;
        ...
        return Consumer<PaginationProvider>(builder: (context, value, child) {
          return ClipRRect(
            ...
            child: PaginatedDataTable(
                rowsPerPage:
                    (orderPlacedList.length >= 5 && orderPlacedList.isNotEmpty)
                        ? 5
                        : (orderPlacedList.isEmpty)
                            ? 1
                            : orderPlacedList.length,
...
class MyData extends DataTableSource {
  ...
  MyData(this.data, this.orderPlaced, this._datetime, this.dealerId,
      this.dealerName, this._showDatePicker,
      {required this.myGlobalBuildContext});

  @override
  int get rowCount => orderPlaced.length;
  ...
  DoorOrderPlacedTable table = DoorOrderPlacedTable();
  ...
  DataRow getRow(int index) {
    ...
    final OrdersModel result = orderPlaced[index];
