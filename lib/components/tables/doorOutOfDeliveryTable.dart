...
class DoorOutOfDeliveryTable extends StatefulWidget {
  final String? dealerId;
  final String? dealerName;
  const DoorOutOfDeliveryTable({super.key, this.dealerId, this.dealerName});

  @override
  State<DoorOutOfDeliveryTable> createState() => _DoorOutOfDeliveryTableState();
}

class _DoorOutOfDeliveryTableState extends State<DoorOutOfDeliveryTable> {
      ...
      builder: (context, snapshot) {
        ...
        return Consumer<PaginationProvider>(builder: (context, value, child) {
          return PaginatedDataTable(
              rowsPerPage: (doorOutForDeliveryList.length >= 5 &&
                      doorOutForDeliveryList.isNotEmpty)
                  ? 5
                  : (doorOutForDeliveryList.isEmpty)
                      ? 1
                      : doorOutForDeliveryList.length,
              ...
              source: MyData(list, doorOutForDeliveryList, _dateTime,
...
class MyData extends DataTableSource {
  List<OrdersModel> doorOutForDeliveryList;
  final String? dealerId;
  final String? dealerName;
  NetworkApiServices apiServices = NetworkApiServices();
  DateTime _datetime = DateTime.now();
  ...
  MyData(this.data, this.doorOutForDeliveryList, this._datetime, this.dealerId,
      this.dealerName, this._showDatePicker,
      {required this.myGlobalBuildContext});

  @override
  int get rowCount => doorOutForDeliveryList.length;
  ...
  DataRow getRow(int index) {
    ...
    final OrdersModel result = doorOutForDeliveryList[index];
