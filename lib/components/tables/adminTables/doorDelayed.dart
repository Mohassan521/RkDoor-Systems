...
class AdminDoorDelayed extends StatefulWidget {
  final String dealerId;
  final String dealerName;
  final String? role;
  const AdminDoorDelayed(
      {super.key, required this.dealerId, required this.dealerName, this.role});

  @override
  State<AdminDoorDelayed> createState() => _AdminDoorDelayedState();
}

class _AdminDoorDelayedState extends State<AdminDoorDelayed> {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        ...
        // List<AdminPanelOrders> filteredList = list!.where((result) => result.orderStatusVal == "Delayed").toList();
...
//43
  ...
  int get totalRowCount {
    ...
    for (var dealerData in dealerDataList!) {
      for (var quote in dealerData.orders) {
        if (quote.orderStatusVal == 'Delayed') {
          count++;
        }
      }
    }
