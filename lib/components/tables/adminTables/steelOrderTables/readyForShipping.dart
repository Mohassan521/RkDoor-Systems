...
class AdminSteelReadyForShipping extends StatefulWidget {
  final String dealerId;
  final String dealerName;
  final String? role;
  const AdminSteelReadyForShipping(
      {super.key, required this.dealerId, required this.dealerName, this.role});

  @override
  State<AdminSteelReadyForShipping> createState() =>
      _AdminSteelReadyForShippingState();
}

class _AdminSteelReadyForShippingState
    extends State<AdminSteelReadyForShipping> {
  ...
  void searchSteelOrders(String query) {
    setState(
      () {
        searchInput = query;
        futureSteelOrders =
            apiServices.getSteelOrdersForAdmin(searchQuery: query);
      },
    );
  }
      ...
      children: [
        Container(
            ...
            child: TextFormField(
              ...
              decoration: InputDecoration(
                ...
                hintText: 'Search by Username or Quotation No.',
              ),
            )),
        SizedBox(
          height: 20,
        ...
        FutureBuilder(
          ...
          builder: (context, snapshot) {
            ...
            // List<AdminSteelOrder> filteredList = list!.where((element) => element.steelOrderStatusVal == "Awaiting Deposit").toList();
    ...
    for (var dealerData in dealerDataList!) {
      for (var quote in dealerData.steelOrders!) {
        ...
        List<AdminSteelOrder> getFilteredQuotes() {
          ...
          for (var dealerData in dealerDataList!) {
            for (var quote in dealerData.steelOrders!) {
              if (quote.steelOrderStatusVal == "Ready For Shipping") {
