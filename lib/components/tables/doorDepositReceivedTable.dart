...
class DoorDepositReceivedTable extends StatefulWidget {
  final String? dealerId;
  final String? dealerName;
  const DoorDepositReceivedTable({super.key, this.dealerId, this.dealerName});

  @override
  State<DoorDepositReceivedTable> createState() =>
      _DoorDepositReceivedTableState();
}

class _DoorDepositReceivedTableState extends State<DoorDepositReceivedTable> {
    ...
    return FutureBuilder<List<OrdersModel>>(
      ...
      builder: (context, snapshot) {
        ...
        List<OrdersModel>? displayData =
            filteredList.isNotEmpty ? filteredList : depositReceivedList;
        ...
        return Consumer<PaginationProvider>(builder: (context, value, child) {
          return ClipRRect(
            ...
            child: PaginatedDataTable(
                rowsPerPage: (depositReceivedList.length >= 5 &&
                        depositReceivedList.isNotEmpty)
                    ? 5
                    : (depositReceivedList.isEmpty)
                        ? 1
                        : depositReceivedList.length,
...
class MyData extends DataTableSource {
  List<OrdersModel> depositReceivedList;
  final String? dealerId;
  final String? dealerName;
  NetworkApiServices apiServices = NetworkApiServices();
  DateTime _datetime = DateTime.now();
  ...
  MyData(this.data, this.depositReceivedList, this._datetime, this.dealerId,
      this.dealerName, this._showDatePicker,
      {required this.myGlobalBuildContext});

  @override
  int get rowCount => depositReceivedList.length;
  ...
  //DoorDepositReceivedTable table = DoorDepositReceivedTable();
  ...
  DataRow getRow(int index) {
    ...
    final OrdersModel result = depositReceivedList[index];
