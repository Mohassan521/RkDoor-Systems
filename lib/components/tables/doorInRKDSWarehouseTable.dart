...
class DoorInRKDSTable extends StatefulWidget {
  final String? dealerId;
  final String? dealerName;
  const DoorInRKDSTable({super.key, this.dealerId, this.dealerName});

  @override
  State<DoorInRKDSTable> createState() => _DoorInRKDSTableState();
}

class _DoorInRKDSTableState extends State<DoorInRKDSTable> {
    ...
    return FutureBuilder<List<OrdersModel>>(
      ...
      builder: (context, snapshot) {
        ...
        List<OrdersModel>? displayData =
            filteredList.isNotEmpty ? filteredList : inRKDSWarehouseList;
        ...
        return Consumer<PaginationProvider>(builder: (context, value, child) {
          return ClipRRect(
            ...
            child: PaginatedDataTable(
                rowsPerPage: (inRKDSWarehouseList.length >= 5 &&
                        inRKDSWarehouseList.isNotEmpty)
                    ? 5
                    : (inRKDSWarehouseList.isEmpty)
                        ? 1
                        : inRKDSWarehouseList.length,
...
class MyData extends DataTableSource {
  List<OrdersModel> inRKDSWarehouseList;
  final String? dealerId;
  final String? dealerName;
  NetworkApiServices apiServices = NetworkApiServices();
  DateTime _datetime = DateTime.now();
  ...
  MyData(this.data, this.inRKDSWarehouseList, this._datetime, this.dealerId,
      this.dealerName, this._showDatePicker,
      {required this.myGlobalBuildContext});

  @override
  int get rowCount => inRKDSWarehouseList.length;
  ...
  DataRow getRow(int index) {
    ...
    final OrdersModel result = inRKDSWarehouseList[index];
